...
ANSIBLE_VAULT_FILE="/home/${USER}/qubinode_navigator/inventories/localhost/group_vars/control/vault.yml"
...
function update_profiles_file {
  ...
  python3 profile_generator/profile_generator.py $COMMAND rhel9 rhel9/template.yaml --image rhel-baseos-9.1-x86_64-kvm.qcow2 --user $USER --user-password ${PASSWORD}
  python3 profile_generator/profile_generator.py $COMMAND fedora37 fedora37/template.yaml --image Fedora-Cloud-Base-37-1.7.x86_64.qcow2  --disk-size 30 --numcpus 4 --memory 8192 --user  $USER  --user-password ${PASSWORD}
